...
import api, { getRemedyPrediction } from "../api";// import axios instance
...
const HomeRemedies = () => {
  ...
  /* ML Prediction */
  const handlePredictRemedy = async () => {
  if (!selectedSymptom) return alert("Select symptom first!");

  try {
    const symptoms = [
      selectedSymptom.toLowerCase().replaceAll(" ", "_"),
    ];

    const res = await getRemedyPrediction(symptoms);

    setMlResult(res);
  } catch (err) {
    console.error("ML Error:", err);
    alert("ML API not working");
  }
};
...
const filteredRemedies = remedies.filter((remedy) => {
  if (mlResult) {
    return remedy.title
      .toLowerCase()
      .includes(mlResult.remedy.toLowerCase());
  }
  return true;
});
...
{mlResult && (
  ...
  >
    <h3 style={{ color: "#2f855a", marginBottom: "10px" }}>
    ...
    <div style={{ marginTop: "10px" }}>
      <p><strong>Ingredients:</strong> {mlResult.details.ingredients}</p>
      <p><strong>Instructions:</strong> {mlResult.details.instructions}</p>
      <p><strong>Benefits:</strong> {mlResult.details.benefits}</p>
    </div>
  </div>
)}
